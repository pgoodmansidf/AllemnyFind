// SearchHistory.tsx - COMPLETE FIXED VERSION WITH STAR FUNCTIONALITY
...
export const SearchHistory: React.FC<SearchHistoryProps> = ({ onSelectSearch }) => {
  ...
  const handleSearchSelect = (e: React.MouseEvent, search: SearchResponse) => {
    e.stopPropagation();
    // Don't close the sidebar when selecting a search
    onSelectSearch(search);
    // Keep the sidebar open but collapse it to save space
    if (!isCollapsed) {
      setIsCollapsed(true);
    }
  };
